...
-   `pool` is a function everytime it is called, it returns a connection wrapper.

-   `conn` will be put back into `pool` when exiting `with` statement.

-   If there is exception raise from inside `with` statement, conn will not be put
    back but instead, it will be closed.

-   To get stat about pool:
